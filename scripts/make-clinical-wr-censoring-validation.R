...
simArm <- function(n,p,seed){ set.seed(seed); W<-rnorm(n)
  ...
  C<-rexp(n,cc); cap<-pmin(C,tau)                                  # random censoring
  est<-data.table(W=W,d01=as.integer(hfh&u0<cap),t01=ifelse(hfh&u0<cap,u0,Inf),
    d02=as.integer(!hfh&u0<cap),t02=ifelse(!hfh&u0<cap,u0,Inf),
    d12=as.integer(hfh&u0<cap&dpost<cap),t12=ifelse(hfh&u0<cap&dpost<cap,dpost,Inf),
    exit0=pmin(u0,cap),entry1=ifelse(hfh&u0<cap,s,Inf),exit1=ifelse(hfh&u0<cap,pmin(dpost,cap),Inf),
    obsT=pmin(D,C,tau), censE=as.integer(C<D & C<tau))
  bf<-data.table(D=ifelse(D<=tau,D,Inf),s=ifelse(hfh&s<=tau,s,Inf),state=ifelse(D<=tau,2L,ifelse(hfh&s<=tau,1L,0L)))  # UNCENSORED truth
  list(est=est,bf=bf) }
...
armFit <- function(est){
  ...
  fc<-fitTransitionSL(rep(0,n),est$obsT,est$censE,Cov,grid,SL.library=SLlib,V=5)   # censoring hazard
  Glag<-pmax(rbind(1,exp(-apply(predictTransitionSL(fc,Cov),2,cumsum)))[1:M,,drop=FALSE], Gmin)  # G(t_{k-1})
  ...
  DS0<-matrix(0,Mp1,n); cumdM0<-rbind(0,apply(dM0/(S0end*Glag),2,cumsum))  # cumulative to grid j (IPCW)
