...
#'
#' @return Object with S3 class `"ConcreteEst"`. The fitted object stores TMLE
#' convergence metadata, including the stopping rule, update trace, and empirical
#' EIC norm trajectory. Use [getTmleDiagnostics()] to inspect these diagnostics.
#'
                        ...
                        EICStopRule = "relative", EICStopAbsTol = 0, CrossFit = FALSE, ...)
{
    ...
    # initial estimation ------------------------------------------------------------------------
    InitFn <- if (isTRUE(CrossFit)) getCVInitialEstimate else getInitialEstimate
...
print.ConcreteEst <- function(x, ...) {
    `.` <- `..a` <- `Pt Est` <- se <- PnEIC <- StopCriteria <- ratio <- check <-
        `|Pn EIC| / Stop Criteria` <- `seEIC/(sqrt(n)log(n))` <- NULL
    ...
    cat("EIC stopping rule: ", attr(x, "EICStopRule"),
        ifelse(identical(attr(x, "EICStopRule"), "relative"), "",
               paste0(" (absolute tolerance = ",
                      signif(attr(x, "EICStopAbsTol"), 4), ")")),
        "\n\n", sep = "")
    if (!(isTRUE(attr(x, "TmleConverged")$converged))) {
        ...
        StopDT <- makeOneStepStop(
            SummEIC = StopDT,
            EICStopRule = attr(x, "EICStopRule"),
            EICStopAbsTol = attr(x, "EICStopAbsTol")
        )
        data.table::setnames(StopDT, "Trt", "Intervention")
        PnEICs <- merge(
            PnEICs,
            StopDT[, .(Intervention, Time, Event, StopCriteria, ratio, check)],
            by = c("Intervention", "Time", "Event")
        )
        PnEICs[, "|Pn EIC| / Stop Criteria" := ratio]
        PnEICs <- PnEICs[!check, .SD,
