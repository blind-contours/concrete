...
- EICStopRule:
  ...
  `|PnEIC| <= max(seEIC / (sqrt(n) log(n)), EICStopAbsTol)`, which is
  useful when rare-event or near-zero-variance components make the
  relative criterion numerically too strict.
